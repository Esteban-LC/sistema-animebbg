...
// Los capitulos pueden tener muchas imagenes pesadas, aumentar timeout
...
export async function GET(request) {
    try {
        ...
        // Solo redrawers (y admins/lideres supervisando) pueden descargar
        const rol = String(asignacion.rol || '').toLowerCase();
        if (!isAdmin && !canViewAsLeader && rol !== 'redrawer') {
            return NextResponse.json({ error: 'Solo redrawers pueden descargar los raws' }, { status: 403 });
        }

        if (!asignacion.proyecto_id || asignacion.capitulo === null || asignacion.capitulo === undefined) {
            return NextResponse.json({ error: 'La asignacion no tiene capitulo asignado' }, { status: 400 });
        }

        const proyecto = await db.prepare(`
            SELECT id, titulo, tipo, capitulos_catalogo, drive_folder_id
        ...
        if (!rawUrl) {
            return NextResponse.json({ error: 'No hay raws disponibles para este capitulo' }, { status: 404 });
        }
        ...
        // Obtener listado de imagenes del folder
        ...
        if (imageFiles.length === 0) {
            return NextResponse.json({ error: 'No se encontraron imagenes en el capitulo' }, { status: 404 });
        }

        // Descargar todas las imagenes en paralelo (max 5 a la vez)
        const CONCURRENCY = 5;
        const zipEntries = {};
        const chunks = [];
        ...
        const zipped = zipSync(zipEntries, { level: 0 }); // level 0 = sin compresion (imagenes ya comprimidas)
        const proyectoTitulo = String(proyecto?.titulo || 'proyecto').replace(/[^a-zA-Z0-9áéíóúñÁÉÍÓÚÑ _-]/g, '');
        const cap = String(asignacion.capitulo).padStart(3, '0');
        const variantLabel = variant === 'ENG' ? 'ENG' : 'RAW';
        const filename = `${proyectoTitulo}_Cap${cap}_${variantLabel}.zip`;
